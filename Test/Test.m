...
%% set / add / sub (instance)
v = MVector().set(7,8);         assertEqual(v.get(), [7 8], tol);
v.set(1,2,3);                   assertEqual(v.get(), [1 2 3], tol);
v.set([9 10]);                  assertEqual(v.get(), [9 10], tol);
v.set(MVector([2 3 4]));        assertEqual(v.get(), [2 3 4], tol);

v = MVector(1,2);
v.add(5,6);                     assertEqual(v.get(), [6 8], tol);
v.add([1 1 1]);                 assertEqual(v.get([0 0 0]), [7 9 1], tol);
v2 = MVector(1,1,1);
v.add(v2);                      assertEqual(v.get(), [8 10 2], tol);

v = MVector(10,10);
v.sub(1,2);                     assertEqual(v.get(), [9 8], tol);
v.sub([1 1 1]);                 assertEqual(v.get([0 0 0]), [8 7 -1], tol);
v.sub(MVector(1,2,3));          assertEqual(v.get(), [7 5 -4], tol);

%% mult/div, mag, magSq, dist, dot, cross (instance)
v = MVector(1,2).mult(3);       assertEqual(v.get(), [3 6], tol);
v = MVector(1,2,3).mult(2);     assertEqual(v.get(), [2 4 6], tol);
v = MVector(2,4,6).div(2);      assertEqual(v.get(), [1 2 3], tol);
...
a = MVector(0,0,0); b = MVector(1,2,2);             assert(abs(a.dist(b) - 3) < tol);
...
v = MVector(3,4).limit(2);        assert(abs(v.mag()-2) < 1e-12);
v = MVector(1,2,2).limit(2);      assert(abs(v.mag()-2) < 1e-12);
v = MVector(0,0).limit(0);        assertEqual(v.get(), [0 0], tol);
...
c = MVector.addS(a,b);                 assertEqual(c.get(), [4 6], tol);
t = MVector(); MVector.addS(a,b,t);    assertEqual(t.get(), [4 6], tol);

c = MVector.subS(b,a);                 assertEqual(c.get(), [2 2], tol);
MVector.subS(b,a,t);                   assertEqual(t.get(), [2 2], tol);

d = MVector.multS(MVector(1,2,3), 2);  assertEqual(d.get(), [2 4 6], tol);
MVector.multS(MVector(1,2), 3, t);     assertEqual(t.get(), [3 6], tol);

e = MVector.divS(MVector(2,4,6), 2);   assertEqual(e.get(), [1 2 3], tol);
MVector.divS(MVector(3,6), 3, t);      assertEqual(t.get(), [1 2], tol);

dd = MVector.distS(MVector(0,0), MVector(3,4));      assert(abs(dd - 5) < 1e-12);
sd = MVector.dotS(MVector(1,2,3), MVector(4,5,6));   assert(abs(sd - 32) < 1e-12);

tx = MVector(); MVector.crossS(MVector(1,0,0), MVector(0,1,0), tx); assertEqual(tx.get(), [0 0 1], tol);

lf = MVector.lerpS(MVector(0,0), MVector(10,0), 0.25);  assertEqual(lf.get(), [2.5 0], 1e-12);
lf = MVector.lerpS(MVector(0,0,0), MVector(0,0,8), 0.5); assertEqual(lf.get(), [0 0 4], 1e-12);
